...
/**
 * Core permission functions for RBEC
 */
if (!function_exists('rbec_user_can_see_edit_button')) {
    function rbec_user_can_see_edit_button() {
        static $cache = array();
        $user_id = get_current_user_id();
        if (isset($cache['edit'][$user_id])) {
            return $cache['edit'][$user_id];
        }
        
        $user = wp_get_current_user();
        $can_see = true; // Default to true
...
if (!function_exists('rbec_user_can_see_elementor_button')) {
    function rbec_user_can_see_elementor_button() {
        static $cache = array();
        $user_id = get_current_user_id();
        if (isset($cache['elementor'][$user_id])) {
            return $cache['elementor'][$user_id];
        }
        ...
        $cache['elementor'][$user_id] = $can_see;
        return $can_see;
    }
}
